...
contract ExponentialCurveTest is DSTest {
    ...
    function test_getBuyInfoExample() public {
        ...
        (CurveErrorCodes.Error error, ICurve.Params memory newParams, uint256 inputValue, ICurve.Fees memory fees) =
        curve.getBuyInfo(
            ICurve.Params(spotPrice, delta, "", ""),
            numItems,
            ICurve.FeeMultipliers(feeMultiplier, protocolFeeMultiplier, royaltyNumerator, 0)
        );
    ...
    function test_getBuyInfoWithoutFee(uint128 spotPrice, uint64 delta, uint8 numItems) public {
        ...
        (CurveErrorCodes.Error error, ICurve.Params memory newParams, uint256 inputValue,) =
        ...
        uint256 fullWidthNewSpotPrice = uint256(spotPrice).fmul(deltaPowN, FixedPointMathLib.WAD);
        if (fullWidthNewSpotPrice > type(uint128).max) {
            ...
            if (spotPrice > 0 && numItems > 0) {
                assertTrue(
                    (newParams.spotPrice > spotPrice && delta > FixedPointMathLib.WAD)
                        || (newParams.spotPrice == spotPrice && delta == FixedPointMathLib.WAD),
    ...
    function test_getSellInfoExample() public {
        ...
        uint256 protocolFeeMultiplier = (FixedPointMathLib.WAD * 3) / 1000; // 0.3%
        uint256 royaltyNumerator = 0;
        (CurveErrorCodes.Error error, ICurve.Params memory newParams, uint256 outputValue, ICurve.Fees memory fees) =
        curve.getSellInfo(
            ICurve.Params(spotPrice, delta, "", ""),
            numItems,
            ICurve.FeeMultipliers(feeMultiplier, protocolFeeMultiplier, royaltyNumerator, 0)
    ...
    function test_getSellInfoWithoutFee(uint128 spotPrice, uint128 delta, uint8 numItems) public {
        ...
        (CurveErrorCodes.Error error, ICurve.Params memory newParams, uint256 outputValue,) =
            curve.getSellInfo(ICurve.Params(spotPrice, delta, "", ""), numItems, ICurve.FeeMultipliers(0, 0, 0, 0));
        assertEq(uint256(error), uint256(CurveErrorCodes.Error.OK), "Error code not OK");

        if (spotPrice > MIN_PRICE && numItems > 0) {
            assertTrue(
                (newParams.spotPrice < spotPrice && delta > 0) || (newParams.spotPrice == spotPrice && delta == 0),
                "Price update incorrect"
            );
        }
