...
describe("RewardVaultETHDraw", function () {
  for (const [numDistinctERC721Tokens, numPerERC721Token] of [
    [3, 10],
    [0, 0],
  ]) {
    const hasNoERC721 =
      numDistinctERC721Tokens === 0 && numPerERC721Token === 0;
    const title = hasNoERC721 ? "Without ERC721" : "With ERC721";
    describe(title, function () {
      ...
      async function rewardVaultDrawFixture() {
        ...
        const tx = await collectionstaker.connect(owner).createIncentiveETHDraw(
          ...
          endTime,
          {
            nftCollectionsPrize: shuffledNFTAddresses,
            nftIdsPrize: shuffledNFTTokenIds,
      ...
      describe("Deployment", function () {
        ...
        it("Should have correct state variables", async function () {
          const { rewardVault, user } = await loadFixture(
            rewardVaultDrawFixture
          );
      ...
      describe("Resolving draw close", function () {
        it("Should be able to close and resolve (sweep remainder)", async function () {
          ...
          await rewardVault
            .connect(owner)
            .setPrizePerWinner(thisPrizePerWinner); // 30 % 7 == 2
          const thisRemainder =
          ...
          await rewardVault
            .connect(owner)
            .sweepUnclaimedNfts(epoch, remainderNfts);
          // Loop through distinctNFTs
          ...
          expect(afterBalance.sub(beforeBalance)).to.equal(thisRemainder);

          const txPromise = rewardVault
            .connect(owner)
            .sweepUnclaimedNfts(epoch, remainderNfts);
          if (remainderNfts.length) {
            // Cannot sweep unclaimed nfts again for the same epoch
            // as the nfts would have already been transferred out in the previous sweepUnclaimedNfts call.
            await expect(txPromise).to.be.revertedWith(
              "ERC721: caller is not token owner nor approved"
            );
          } else {
            // No nfts transferred, no error.
            await expect(txPromise).to.not.be.revertedWith(
              "ERC721: caller is not token owner nor approved"
            );
          }
        });
        ...
        it("Should be able to close and resolve", async function () {
          ...
          expect(
            await rewardVault.isPrizeClaimable(epoch, user.address)
          ).to.be.true;
          ...
          expect(
            await rewardVault.isPrizeClaimable(epoch, user1.address)
          ).to.be.true;
          ...
          for (let i = 0; i < distinctNFTs.length; i++) {
            ...
            expect(
              await distinctNFTs[i].balanceOf(rewardVault.address)
            ).to.equal(0);
        ...
        it("Balance changes (staking/burning) that occur while the draw is closed are reflected in the next epoch (e.g. epoch 1 > balance change > epoch 2", async function () {
          ...
          const { newPrizeNFTAddresses, newPrizeTokenIds } =
            await setUpNewPrizeNFT(numIncrementalPrizes, owner, rewardVault);
      ...
      describe("Multiple rounds of running a draw", function () {
        it("It should be able to go through multiple rounds of draw, without rewards programme (same)", async function () {
          ...
          for (let i = 0; i < 3; i++) {
            ...
            // Close the draw
            await time.increase(rewardDuration);
            if (hasNoERC721 && expectedEpoch === 1) {
              await rewardVault.connect(owner).setPrizePerWinner(40); // This sets the divisbility of the prize
            }
      ...
      describe("Can add prizes (access roles)", function () {
        it("Should be able to add new prizes by the deployer of the vault (assume factory is contract)", async function () {
          const { owner, rewardVault } = await loadFixture(
            rewardVaultDrawFixture
          );
        ...
        it("Should not be able to add new prizes by any other person", async function () {
          const { rewardVault, user1 } = await loadFixture(
            rewardVaultDrawFixture
          );
          //   Const { nft: newPrizeNFT } = await nftFixture();
          const numNewPrizes = 10;
          ...
          const { newPrizeNFTAddresses, newPrizeTokenIds } =
            await setUpNewPrizeNFT(numNewPrizes, newNFTOwner, rewardVault);

          //   //   Add the erc721

          const rewardTokenAmount = ethers.utils.parseEther("100");
          const { rewardTokensNew, rewardTokenAmountList } =
            await setUpNewPrizeERC20(
              newNFTOwner,
              rewardVault,
              rewardTokenAmount
            );
          ...
          await expect(
            rewardVault.connect(newNFTOwner).addNewPrizes({
              nftCollectionsPrize: newPrizeNFTAddresses,
              nftIdsPrize: newPrizeTokenIds,
              erc20Prize: rewardTokensNew.map(
