 ...
 */
contract ExponentialCurveWithSpreadInflator is CurveWithSpreadInflator, CurveErrorCodes {
        ...
        returns (Params memory newParams, uint256 inputValue, Fees memory fees, uint256 lastSwapPrice)
    {
        ...
        // NB: this is ugly but (A) we faced stack too deep and (B) this saves gas over using applyInflator() due to jump call (400 more gas)

        inputValue = bidAskInflatorIsFlatAmount
            ? (
                rawAmount.fmul(delta, FixedPointMathLib.WAD) * (deltaPowN - FixedPointMathLib.WAD)
            ...
            : (
                (
                    rawAmount.fmul(delta, FixedPointMathLib.WAD) * (deltaPowN - FixedPointMathLib.WAD)
                        / (delta - FixedPointMathLib.WAD)
                ) * (INFLATOR_DENOMINATOR + inflator)
            ) / INFLATOR_DENOMINATOR;
